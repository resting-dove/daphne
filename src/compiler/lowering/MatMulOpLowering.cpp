...
public:
  ...
  matchAndRewrite(daphne::MatMulOp op, OpAdaptor adaptor,
    ...
    if (options.tile && is_tileable(rhsMemRefType.getShape())) {
      ...
      llvm::SmallVector<AffineForOp> tiled_loops =
          tile_loops(loops, tile_sizes);
      AffineCopyOptions options;
      options.fastMemCapacityBytes = 32;
      options.fastMemorySpace = 1;
      options.slowMemorySpace = 0;
      options.generateDma = false;
      (void)affineDataCopyGenerate(tiled_loops[2], options, lhs,
                                   *new DenseSet<Operation *>);
      options.fastMemCapacityBytes = 16;
      (void)affineDataCopyGenerate(tiled_loops[3], options, rhs,
                                   *new DenseSet<Operation *>);
    }
  ...
  // loops.
  llvm::SmallVector<AffineForOp>
  tile_loops(SmallVector<AffineForOp, 3> loops,
    ...
    llvm::SmallVector<AffineForOp> jammedNest;
    getPerfectlyNestedLoops(jammedNest, blisTiledLoops.front());

    if (failed(loopUnrollUpToFactor(jammedNest.back(), KU))) {
      spdlog::warn("Could not unroll the K loop in MatMulLowering");
    }
    int64_t i = 0;
    /* while (succeeded(promoteIfSingleIteration(jammedNest[i])) && i < 4) {
        i++;
    } */
    llvm::SmallVector<AffineForOp> lastNest;
    getPerfectlyNestedLoops(lastNest, jammedNest[i]);
    return lastNest;
  }
};
